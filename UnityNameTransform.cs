...
namespace TouhouMix {
	...
	/// </summary>
	public class UnityNameTransform : INameTransform {
		/// <summary>
		...
		/// <summary>
		/// Initialises a new instance of <see cref="UnityNameTransform"/>
		/// </summary>
		/// <param name="baseDirectory"></param>
		/// <param name="allowParentTraversal">Allow parent directory traversal in file paths (e.g. ../file)</param>
		public UnityNameTransform(string baseDirectory, bool allowParentTraversal = false) {
		...
		/// <summary>
		/// Initialise a default instance of <see cref="UnityNameTransform"/>
		/// </summary>
		public UnityNameTransform() {
			// Do nothing.
		}
			...
			if (name != null) {
				...
				if (_baseDirectory != null) {
					...
					if (!_allowParentTraversal && !Path.GetFullPath(name).StartsWith(_baseDirectory, StringComparison.InvariantCultureIgnoreCase)) {
						throw new Exception("Parent traversal in paths is not allowed");
					}
				}
			} else {
				name = string.Empty;
